...
void createToken(Token token[256],int row_no,int col_no,char *str,int idno) {
...
void getArithOp(char line[],int row_no,Token token[256]) {
    for(int i=0;i<strlen(line);i++) {
        ...
        if(ArithOp(line[i]) && !ArithOp(line[i+1]) && !ArithOp(line[i-1]) && (i+1)<strlen(line)) {
            if(line[i] == '+') {
                createToken(token,row_no,i,"ARTHOP: +",0);
            }
            else if(line[i] == '-') {
                createToken(token,row_no,i,"ARTHOP: -",0);
            }
...
void getLogicalOp(char line[],int row_no,Token token[256]) {
    for(int i=0;i<strlen(line);i++) {
        ...
        if((i+1) < strlen(line)) {
            if(line[i] == '&' && line[i+1] == '&') {
                createToken(token,row_no,i,"LgOP: AND",0);
                i++;
            }
            else if(line[i] == '|' && line[i] == '|') {
                createToken(token,row_no,i,"LgOP: OR",0);
                i++;
            }
            else if(line[i] == '!' && !LogicalOp(line[i+1])) {
                createToken(token,row_no,i,"LgOP: NOT",0);
                i++;
            }
        }
    }
...
void getRelOp(char line[],int row_no,Token token[256]) {
    for(int i=0;i<strlen(line);i++) {
        ...
        if((i+1) < strlen(line)) {
            if(line[i] == '!' && line[i+1] == '=') {
                createToken(token,row_no,i,"Relop:NE",0);
                i++;
            }
            else if(line[i] == '=' && line[i+1] == '=') {
                createToken(token,row_no,i,"Relop:EQ",0);
                i++;
            }
            else if(line[i] == '<' && line[i+1] == '=') {
                createToken(token,row_no,i,"Relop:LE",0);
                i++;
            }
            else if(line[i] == '>' && line[i+1] == '=') {
                createToken(token,row_no,i,"Relop:GE",0);
                i++;
            }
            else if(line[i] == '<' && !RelOp(line[i+1])) {
                createToken(token,row_no,i,"Relop:LT",0);
                i++;
            }
            else if(line[i] == '>' && !RelOp(line[i+1])) {
                createToken(token,row_no,i,"Relop:GT",0);
                i++;
            }
        }
    }
...
void getStringLiteral(char line[],int row_no,Token token[256]) {
    for(int i=0;i<strlen(line);i++) {
        ...
        strcat(str,literal);
        createToken(token,row_no,i,str,0);
        i = end_ind;
    }
}
...
void getIdentifiers(char line[], int row_no, Token token[256]) {
    ...
    while (i < strlen(line)) {
        ...
        if (!isKeyword(identifier)) {
            ...
            createToken(token, row_no, start_ind,str,xy);
        }
    }
}
...
void getNumCon(char line[], int row_no, Token token[256]) {
    ...
    while (i < strlen(line)) {
        ...
        char str[256] = {"NumCon : "};
        strcat(str,numCon);
        createToken(token, row_no, start_ind,str,0);
    }
}
...
void getSpclSym(char line[],int row_no,Token token[256]) {
    for(int i=0;i<strlen(line);i++) {
        ...
        if((i+1) < strlen(line)) {
            if(line[i] == '$') {
                createToken(token,row_no,i,"SpclSym: $",0);
                i++;
            }
            else if(line[i] == '%') {
                createToken(token,row_no,i,"SpclSym: %",0);
...
int main() {
    ...
    sortTokens(token);
    for(int i = 0; i < ind; i++) {
        if(strstr(token[i].type,"ID: ")!=NULL){
        printf("\n<%d%s, %d, %d>",token[i].idno,token[i].type,token[i].row_no,token[i].col_no);   
        }
        else
